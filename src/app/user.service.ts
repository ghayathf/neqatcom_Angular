...
export class UserService {
  ...
  async UpdateUser(user: any) {
    ...
    if (this.ImgaeName != null)
      user.userimage = this.ImgaeName
    else
      user.userimage = this.userById.userimage
...
ImgaeName :any;
  UploadImage(imageFile: any) {
    this.spinner.show()
    return new Promise<void>((resolve, reject) => {
      this.http.post(`${environment.apiUrl}/User/UploadImage`, imageFile).subscribe(
        {
          next: (res: any) => {
            this.spinner.hide()
  ...
  GetFollowers(lendId: any) {
    this.spinner.show()
    return new Promise<void>((resolve, reject) => {
      this.http.get(`${environment.apiUrl}/User/GetAllFollower/` + lendId).subscribe(
        {
          next: (res) => {
  ...
  AddFollower(lendId: any, loaneId: any) {
    this.spinner.show()
    return new Promise<void>((resolve, reject) => {
      this.http.post(`${environment.apiUrl}/User/addfollower/${lendId}/${loaneId}`, {}).subscribe(
        {
          next: (res) => {
            this.spinner.hide()
            resolve()
  ...
  DeleteFollower(lendId: any, loaneId: any) {
    this.spinner.show()
    return new Promise<void>((resolve, reject) => {
      this.http.delete(`${environment.apiUrl}/User/DeleteFollower/${lendId}/${loaneId}`, {}).subscribe(
        {
          next: (res) => {
            this.spinner.hide()
            resolve()
