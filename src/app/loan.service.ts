...
export class LoanService {
  ...
  GetAllRequestedLoans(lenderid: number, statusid: any) {
    return new Promise<void>((resolve, reject) => {
      ...
      this.http
        .get(
          `${environment.apiUrl}/Loan/GetAllStatusLoans/` +
            lenderid +
            '/' +
            statusid
        )
  ...
  deleteLoan(loanId: number) {
    return new Promise<void>((resolve, reject) => {
      this.http
        .delete(`${environment.apiUrl}/Loan/DeleteLoan/` + loanId)
        .subscribe({
          next: () => {
            this.spinner.hide();
  ...
  updateLoanStatus(LoanId: any, status: any) {
    return new Promise<void>((resolve, reject) => {
      this.http
        .put(
          `${environment.apiUrl}/Loan/UpdateLoan/` + LoanId + '/' + status,
          {}
        )
        .subscribe(
          () => {
  ...
  calc() {
    ...
    const monthlyAmountWithTax =
      monthlyAmount + monthlyAmount * paypalTaxRate + paypalTaxFixed;
    const estimatedwithtax =
      estimatedPrice + estimatedPrice * paypalTaxRate + paypalTaxFixed;

    if (estimatedPrice >= monthlyAmountWithTax)
      this.monthly = monthlyAmountWithTax;
    else this.monthly = estimatedPrice;
  ...
  SendFormFromLender(Data: any) {
    ...
    return new Promise<void>((resolve, reject) => {
      this.http
        .put(`${environment.apiUrl}/Loan/ConfirmNewLoanInfo`, Data)
        .subscribe(
          (res) => {
            resolve();
          },
  ...
  ExistingLoanCounter(Loaneeid: any) {
    return new Promise<void>((resolve, reject) => {
      ...
      this.http
        .get(`${environment.apiUrl}/Loan/ExistingLoanCounter/` + Loaneeid)
        .subscribe(
          (res) => {
  ...
  GetAllRequestedPostpones(lenderid: any, statusid: any) {
    return new Promise<void>((resolve, reject) => {
      this.http
        .get(
          `${environment.apiUrl}/Loan/GetAllPostponeRequests/` +
            lenderid +
            '/' +
            statusid
        )
  ...
  requestNewLoan(loaneeid: any, offerid: any, totalmonths: any) {
    return new Promise<void>((resolve, reject) => {
      ...
      this.http
        .post(
          `${environment.apiUrl}/Loan/RequestNewLoan/${loaneeid}/${offerid}/${totalmonths}`,
          {}
        )
        .subscribe(
          (res) => {
  ...
  updatePostponeStatus(LoanId: any, status: any, loaneeid: any) {
    return new Promise<void>((resolve, reject) => {
      this.http
        .put(
          `${environment.apiUrl}/Loan/UpdatePostponeStatus/` +
            LoanId +
            '/' +
            status +
            '/' +
