...
export class MainComponent {
  ...
  tour() {
    this.joyride.startTour({
      steps: ['secondstep', 'forthstep', 'thirdstep', 'fifthstep', 'sixthstep'],
    });
  }
  ...
  async OpenDetailsDialog(lendId: any) {
    ...
    this.filterd = this.followers.find(
      (x: any) => x.lenderid == lendId && x.loaneeid == this.loaneeid
    );
  ...
  async toggleFollow() {
    if (this.followFlag) {
      await this.userService.AddFollower(
        this.lenderId,
        localStorage.getItem('Loaneeid')
      );
      console.log('Followed');
    } else {
      await this.userService.DeleteFollower(
        this.lenderId,
        localStorage.getItem('Loaneeid')
      );
  ...
  async RequestSelectedOffer() {
    ...
    await this.loanservice.requestNewLoan(
      this.loaneeid,
      this.selectedofferid,
      this.selectedtotalmonths
    );
    this.dialog.closeAll();
    await this.ngOnInit();
  }
