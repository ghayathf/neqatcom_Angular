...
export class RegisterService {
  ...
  async CreateLender(Lender: any) {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      this.http
        .post(`${environment.apiUrl}/LenderStore/createLenderStore`, Lender)
        .subscribe({
          next: async () => {
            this.spinner.hide();
  ...
  async CreateLoanee(Loanee: any) {
    ...
    this.nn = this.NationalNumbers.find(
      (x: any) => x.nationalnum == Loanee.nationalnumber
    );
    ...
    if (this.nn != null) {
      ...
      return new Promise<void>((resolve, reject) => {
        this.http
          .post(`${environment.apiUrl}/Loanee/CreateLoanee`, Loanee)
          .subscribe({
            next: () => {
              this.spinner.hide();
            },
    ...
    return new Promise<void>((resolve, reject) => {
      this.http.get(`${environment.apiUrl}/User/GetAllUsers`).subscribe({
        next: (res) => {
          ...
          const maxId = Math.max(
            ...this.tr.map((trainer: { userid: any }) => trainer.userid)
          );
          this.lastid = maxId;
  ...
  extractLenderUsers() {
    this.lenderUsers = this.users.filter(
      (user: { role: string }) => user.role === 'Lender'
    );
  }
  users2: any = [];
  lastid2: any;
    ...
    return new Promise<void>((resolve, reject) => {
      this.http.get(`${environment.apiUrl}/User/GetAllUsers`).subscribe({
        next: (res) => {
          ...
          this.filterUsers2();
          const maxId = Math.max(
            ...this.tr2.map((trainer: { userid: any }) => trainer.userid)
          );
          this.lastid2 = maxId;
  ...
  async getAllNationalNumbers() {
    ...
    return new Promise<void>((resolve, reject) => {
      this.http
        .get(`${environment.apiUrl}/Loanee/GetAllGpnationalnumber`)
        .subscribe({
          next: (res) => {
