...
export class LoanRequestsComponent {
  ...
  @ViewChild('F2fMeetingDialog', { static: true })
  OnsiteMeeting!: TemplateRef<any>;
  ...
  constructor(
    ...
    public loaneeMeetingService: LoaneeService
  ) {}
  ...
  async CreateOnlineMeeting() {
    ...
    try {
      ...
      this.m =
        'We would like to inform you that a meeting has been organized between you and ' +
        this.lenderName +
        ' on ' +
        formattedDate +
        ' at ' +
        this.Meetingtime +
        ' To make an agreement regarding the loan that you applied for, Join the meeting on this link: ' +
        this.link.join_url;
      await this.sendEmail(
        this.selectedLoaneeMail,
        this.selectedLoaneeFname,
        this.m,
        this.s
      );

      this.CreateNotificationForm.controls['userid'].setValue(
        this.AuthGard.userId
      );
      ...
      this.s = 'Online Meeting Appointment';
      this.m =
        'We would like to point out that you have organized an online meeting on ' +
        formattedDate +
        ' at ' +
        this.Meetingtime +
        ' with the customer ' +
        this.selectedLoaneeFname +
        ' ' +
        this.selectedLoaneeLname +
        ' To make an agreement regarding the loan that this loanee applied for, Join the meeting on this link: ' +
        this.link.join_url;
      await this.sendEmail(this.lenderEmail, this.lenderName, this.m, this.s);
      this.CreateNotificationForm.controls['userid'].setValue(
        this.selectedLoaneeuserid
      );
      this.CreateNotificationForm.controls['notificationsmessage'].setValue(
        'New Online Meeting'
      );
  ...
  async CreateOnsilteMeeting() {
    ...
    this.m =
      ...
      ' at ' +
      this.Meetingtime +
      ' To make an agreement regarding the loan that you applied for, Be On time at ' +
      googleMapsUrl;
    await this.sendEmail(
      this.selectedLoaneeMail,
      this.selectedLoaneeFname,
